...
class JwtBuilderService
{
    ...
    {
        return $this->registerClaim('iat', intval($val));
    }
     ...
     * @return $this
     */
    public function expiresAt(CarbonInterface $carbon)
    {
        return $this->registerClaim('exp', intval($carbon->timestamp));
    }
    ...
    public function sessionId($val)
    {
        return $this->registerClaim('sid', $val);
    }

    public function tokenType($val)
    {
        return $this->registerClaim('ttp', $val);
    }

    public function payload(array $val = null)
    {
        return $this->registerClaim('pld', $val);
    }


    public function prv($val)
    {
        return $this->registerClaim('prv', $val);
    }
    ...
    {
        return $this->registerClaim('nbf', intval($carbon->timestamp));
    }
    ...
    {
        return JWT::encode($this->claims, $this->getSecretKey(), $this->getAlgorytm());
    }
    ...
    protected function getAlgorytm()
    {
        return config('jwt.encryption_algorytm');
    }

    protected function registerClaim(string $name, $val): self
    {
